...
class Menu extends MY_Controller
{
     ...
     * @access public
     */
    public function render($menu /*start DXLab*/, $tpl_folder="" /*end DXLab*/) {
        $this->clear();
    ...
    private function _prepare_item_tpl($index = 0, $wrapper = FALSE) {
        ...
        /*start DXLab*/
        if (!$wrapper && $this->cur_level==0) $this->dropidx = $index;
        if ($wrapper && $this->cur_level==1) $this->dropidx++;
        if ($this->cur_level==1 || ($this->cur_level==2 && $this->arranged_menu_array[$index]['has_childs'])) $this->menuidx1++;

        $has_active_child = (((stristr($this->core->current_url, $this->arranged_menu_array[$index]['link'])) && $this->arranged_menu_array[$index]['has_childs'] && $this->arranged_menu_array[$index]['link'].'/'!=base_url()) )? true: false;
        /*end DXLab*/
        $data = [
            ...
            /*start DXLab*/
            'index' => $index,
            'menuidx0' => $this->menuidx0,
            'dropidx' => $this->dropidx,
            'menuidx1' => $this->menuidx1,
